...
export function BallonDorModal({
  player,
  seasonStat,
  onClose
}: {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/90 p-4">
      ...
      >
        ...
        <div className="text-center mb-8 relative z-10">
          ...
          <h2 className="text-3xl font-black text-white">Gala da Bola de Ouro</h2>
          <p className="text-slate-400 mt-2 text-sm">
            Os melhores jogadores do mundo nesta temporada.
          </p>
        </div>

        <div className="space-y-3 mb-8">
          {candidates.map((candidate, idx) => {
            const isMe = candidate.name === player.name;
            const rank = idx + 1;
            ...
            return (
              <div 
                ...
                className={`flex items-center gap-4 p-3 rounded-xl border ${
                  isMe ? 'bg-amber-500/10 border-amber-500/50' : 'bg-slate-800/50 border-slate-700/50'
                }`}
              >
                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black ${
                  rank === 1 ? 'bg-amber-500 text-white' : 
                  rank === 2 ? 'bg-slate-300 text-slate-800' :
                  rank === 3 ? 'bg-amber-700 text-white' : 'bg-slate-700 text-slate-400'
                }`}>
                  {rank}
                </div>
                
                <div className="flex-1">
                  <h4 className={`font-bold ${isMe ? 'text-amber-400' : 'text-slate-200'}`}>
                    {candidate.name} {isMe && '(Você)'}
                  </h4>
                ...
                <div className="text-right">
                  <div className="text-xs text-slate-400 mb-1">Chances</div>
                  <div className={`font-bold text-lg ${
                    candidate.chance > 30 ? 'text-emerald-400' : 
                    candidate.chance > 15 ? 'text-yellow-400' : 'text-slate-300'
                  }`}>
                    {candidate.chance}%
                  </div>
                </div>
